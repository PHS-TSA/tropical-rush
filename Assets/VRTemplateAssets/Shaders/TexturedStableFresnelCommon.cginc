...
half4 fragRimShader(const fresnel_vertex i) : COLOR
{
    const half3 worldViewDir = normalize(UnityWorldSpaceViewDir(i.worldPos));
    const half rim = saturate(
        (1.0 - saturate(dot(normalize(worldViewDir), i.worldNormal)) - _EdgeData.x) / (_EdgeData.y - _EdgeData.x));
    const half processedRim = (3 + _EdgeData.z) * pow(rim, _EdgeData.z + 1) - (2 + _EdgeData.z) * pow(
        rim, _EdgeData.z + 2);
